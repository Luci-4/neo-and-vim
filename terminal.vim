...
function! RunTermCommand(cmd) abort
    call EnsureOpenedSingleTerminal()

    let term_buf_name = "__MY_TERMINAL__"
    let term_buf = bufnr(term_buf_name)

    if term_buf == -1
        echoerr "Terminal buffer not found"
        return
    endif

    let chan = getbufvar(term_buf, 'terminal_job_id', -1)
    if chan == -1
        echoerr "No channel found for terminal buffer"
        return
    endif

    call chansend(chan, a:cmd . "\n")
endfunction
...
function! ToggleSingleTerminal()
    ...
    call setbufvar(bufnr("%"), 'terminal_job_id', b:terminal_job_id)
endfunction

function! EnsureOpenedSingleTerminal()
    if tabpagenr('$') < 2
        tabnew
    else
        if tabpagenr() != 2
